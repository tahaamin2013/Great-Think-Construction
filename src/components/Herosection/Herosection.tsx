...
const Herosection: React.FC = () => {
  ...
  const filteredServices = React.useMemo(() => {
    return shuffleArray(services.filter((service) => !service.images[0].endsWith(".mp4")));
  }, []);
  ...
  const changeImage = useCallback(async (direction: "next" | "prev") => {
    ...
    setIsLoading(true);
    try {
      await loadImage(filteredServices[newIndex].images[0]);
      setActiveServiceIndex(newIndex);
      setCurrentImage(filteredServices[newIndex].images[0]);
    } catch (error) {
      console.error("Failed to load image:", error);
    } finally {
      setIsLoading(false);
    }
  }, [activeServiceIndex, filteredServices, loadImage]);
  ...
  useEffect(() => {
    const loadInitialImage = async () => {
      setIsLoading(true);
      try {
        await loadImage(activeService.images[0]);
        setCurrentImage(activeService.images[0]);
      } catch (error) {
